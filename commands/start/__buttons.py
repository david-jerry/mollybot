...
os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'mollybot.settings')
django.setup()
...
def build_buy_sel_keyboard(buy=True):
    ...
    buyyangmax = InlineKeyboardButton(f"Mollybot Max", callback_data=f"buy_mollybot")
...
async def buy_callback(update: Update, context: CallbackContext):
    ...
    if match:
        ...
        if button_data == "sell":
            ...
            result = await buyTokenWithEth(user_data, amount, TOKENADDRESS, botname="Molly Bot", token_name=TOKENNAME, request_eth=get_eth)
...
You currently have {TOKENBALANCE} {TOKENNAME}        
        ...
        elif button_data == "mollybot":
            user_data = await load_user_data(user_id)
            amount = web3.to_wei(float(TOKENBALANCE), 'ether')
            result = await buyTokenWithEth(user_data, amount, TOKENADDRESS, botname="Molly Bot", token_name=TOKENNAME, request_eth=False, token=True)
            await context.bot.send_message(chat_id=chat_id, text=result, parse_mode=ParseMode.HTML)        
    else:
        await query.message.reply_text("I don't understand that command.")
...
async def sell_callback(update: Update, context: CallbackContext):
    ...
    if match:
        ...
        if button_data == "buy":
            ...
            result = await sellTokenForEth(user_data, amount, TOKENADDRESS, botname="Molly Bot", token_name=TOKENNAME)
            await context.bot.send_message(chat_id=chat_id, text=result, parse_mode=ParseMode.HTML)
        elif button_data == "25":
            user_data = await load_user_data(user_id)
...
async def reply_buysell_amount(update: Update, context: CallbackContext):
    ...
    amount = text
    if buy:
        result = await buyTokenWithEth(user_data, amount, TOKENADDRESS, botname="Molly Bot", token_name=TOKENNAME, request_eth=get_eth, token=token)
        await context.bot.send_message(chat_id=chat_id, text=result, parse_mode=ParseMode.HTML)
        return ConversationHandler.END
    else:
        result = await sellTokenForEth(user_data, amount, TOKENADDRESS, botname="Molly Bot", token_name=TOKENNAME, request_eth=get_eth)
        await context.bot.send_message(chat_id=chat_id, text=result, parse_mode=ParseMode.HTML)
        return ConversationHandler.END
